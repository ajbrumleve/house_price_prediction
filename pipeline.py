...
def run(scrape=False, prebuilt_model=True, rfe=False):
    @log
    def get_realtor_object():
        r = RealtorScraper(page_numbers=206)
        try:
            df_obj = r.create_dataframe()
            r.df = df_obj[0]
    ...
    def find_deals(realtor_obj,model):
        df = realtor_obj.df
        df["prediction"] = df['price'].apply()
    ...
    try:
        if address_price[0] > address_price[1]:
            print(f"The model predicts a price of {address_price[1]}. The actual price is {address_price[0]}. The house is {address_price[0]-address_price[1]} cheaper than the prediction.")
        elif address_price[0] < address_price[1]:
            print(f"The model predicts a price of {address_price[1]}. The actual price is {address_price[0]}. The house is {address_price[1] - address_price[0]} more expensive than the prediction.")
        else:
